...
public class RequestAnalyzer implements RequestHandler, BindingController {
  ...
  public void handleRequest (Request r) throws IOException {
    ...
    if (rq.startsWith("GET ")) {
      ...
      if (r.url.startsWith("reconf/") || r.url.equals("reconf")) {
        if (!r.url.equals("reconf"))
          r.url = r.url.substring("reconf/".length());
        bindFc("rh", new HttpHandler());
      }
      else bindFc("rh", new FileRequestHandler());
      rh.handleRequest(r);
    }
    r.out.close();
    r.s.close();
